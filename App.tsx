      ...
      <div className="w-full max-w-5xl space-y-8 relative">
        ...
        <div className="grid grid-cols-1 gap-8">
          ...
          <div className="grid grid-cols-3 gap-4 animate-slideUp" style={{animationDelay: '0.2s'}}>
            {[
              { name: 'PLANNER', icon: '🧠', status: status === AppStatus.PLANNING, color: 'cyan', desc: 'AI Analysis' },
              { name: 'EXECUTOR', icon: '⚡', status: status === AppStatus.EXECUTING, color: 'amber', desc: 'Tool Runner' },
              { name: 'VERIFIER', icon: '✓', status: status === AppStatus.VERIFYING, color: 'violet', desc: 'Result Synthesis' },
            ].map((agent, i) => (
              <div
                key={agent.name}
              ...
              >
                <div className={`text-2xl mb-2 ${agent.status ? 'animate-bounce-slow' : ''}`}>{agent.icon}</div>
